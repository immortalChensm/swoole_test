...
class Server
{
    ...
    public static function receive(swoole_server $serv, $fd, $from_id, $data)
    {
        ...
        if(strpos($data,":")){
           ...
           //$serv->sendwait($fd,$data);

            $worker_id = self::$workerProcess['worker'.$serv->worker_id];
            echo '当前worker进程是---:'.$worker_id;
            $msg = explode(":",$data);

            $to_worker_id = $msg[0];
            if($to_worker_id==$worker_id){
                $serv->send($fd,'你自己不能给自己发消息');
            }else{
                echo '准备向'.$to_worker_id.'进程发送数据'.PHP_EOL;
                $serv->sendMessage($data,$to_worker_id);
            }
