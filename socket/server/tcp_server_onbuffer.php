...
class Server
{
    ...
    public function start()
    {
        ...
        $this->serv->on("BufferFull",[$this,'onBufferFull']);
    ...
    public function receive(swoole_server $serv,$fd,$reactorId,$data)
    {
        ...
        for($i=0;$i<100;$i++){
            $all = $file.$file.$file.$file.$file;
            $serv->send($fd,"你的数据是：".$all);
        }
    ...
    public function onBufferFull(swoole_server $serv,$fd)
    {
        echo $fd.'---发送缓冲区不够了:'.PHP_EOL;
    }
